...
export const AI_AGENTS = {
  EMAIL_MARKETING: {
    ...
    role: 'Consultor Senior en Marketing para la Industria Acuícola',
    description: 'Experto en redactar campañas persuasivas para productores de camarón y peces.',
    ...
    generatePrompt: (capsule: any, tone: string) => {
      ...
      return `Actúa como un experto consultor senior en marketing para la industria acuícola. 
Tu objetivo es escribir un correo altamente persuasivo y profesional para invitar a productores a ver una cápsula de conocimiento titulada "${capsule.title}".
...
1. Tono: ${tone.toUpperCase()} 
   ...
   - TÉCNICO: Usa terminología precisa (FCA, parámetros, biomasa), enfocado en la ciencia detrás del resultado.
2. Estilo: Evita clichés ("Espero que estés bien", "Saludos cordiales"). Sé directo, valioso y premium.
3. Estructura: Resalta el VALOR ESPECÍFICO que el productor obtendrá al invertir tiempo viendo esta cápsula.
