...
export class CrmService {
  ...
  async findAllContacts(tenantId: string) {
    return this.db.mysql.contact.findMany({
      where: { tenantId },
      include: { 
        _count: { select: { orders: true, leads: true, deals: true, tasks: true } }
      },
  ...
  async upsertContactFromActivity(tenantId: string, email: string, name: string, phone?: string) {
  ...
  async addPoints(contactId: string, tenantId: string, amount: number, reason: string) {
  ...
  async calculateLeadScores(tenantId: string) {
    const contacts = await this.db.mysql.contact.findMany({
      where: { tenantId },
      include: {
        _count: { select: { orders: true, leads: true, deals: true, activities: true, tasks: true } },
    ...
    return contacts.map(contact => {
      ...
      const score = (contact._count.orders * 25) + 
      ...
      if (contact._count.orders > 0) {
        reasons.push(contact._count.orders > 1 ? `Cliente recurrente con ${contact._count.orders} pedidos` : "Realizó su primera compra");
      }
      ...
      if (contact._count.activities > 0) {
        const hasWhatsApp = contact.activities.some(a => a.type === 'WHATSAPP');
        if (hasWhatsApp) reasons.push("Mantiene comunicación activa vía WhatsApp");
      }
      if (contact._count.deals > 0) {
        reasons.push(`Tiene ${contact._count.deals} negociaciones en curso`);
      }
      if (contact._count.tasks > 0) {
        reasons.push(`Tiene ${contact._count.tasks} tareas o citas pendientes`);
      }
  ...
  async getSalesForecast(tenantId: string) {
    ...
    for (let i = -3; i <= 3; i++) {
      ...
      if (isFuture) {
        ...
        const avgPast = Object.values(monthlyRevenue).slice(-3).reduce((a, b) => a + b, 0) / 3 || 5000;
    ...
    return {
      forecastData,
      totalForecastNextMonth: Math.round(forecastData.find(f => f.type === 'PREDICTION')?.revenue || 0),
    ...
    const segments = [
      {
        ...
        description: 'Leads con alto score que aún no han realizado su primer pedido.',
        contacts: scoring.filter(s => s.temperature === 'HOT' && s._count.orders === 0).map(s => s.id)
      },
      {
        id: 'vip_inactive',
        name: 'VIPs Inactivos (30 días)',
        description: 'Clientes VIP que no han tenido actividad registrada en el último mes.',
      ...
      },
      {
        ...
        description: 'Clientes que no han comprado nada en los últimos 60 días.',
        contacts: contacts.filter(c => {
      ...
      },
      {
        ...
        contacts: contacts.filter(c => {
          ...
          return c.status === 'LEAD' && new Date(c.createdAt) > fortyEightHoursAgo;
        }).map(c => c.id)
      }
    ];
