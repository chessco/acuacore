...
| :--- | :--- | :--- |
| [`/src`](./src) | **Obsidian Command Center** (Root App): Frontend principal cargado e integrado dentro de Google AI Studio. | React + Vite + Tailwind CSS |
| [`/api`](./api) | **AcuaCore API Backend**: Lógica central del sistema, endpoints, agentes, skills, base de datos vectorial e integraciones. | NestJS + Prisma + Socket.io |
| [`/web`](./web) | **AcuaCore Web Frontend**: Panel y landing page para el usuario final. Permite gestionar catálogos, leads y configurar el Studio. | React + Vite + PWA |
| [`/cli`](./cli) | **Terminal de Administración**: Consola interactiva para administración rápida, creación de notas/ideas y consultas directas a la IA. | Python + Rich + Httpx |
| [`/docs`](./docs) | Documentación técnica adicional e información del proyecto. | Markdown |
| [`dev.ps1`](./dev.ps1) | Script automatizado en PowerShell para arrancar todo el entorno de desarrollo local. | PowerShell |
| [`DEPLOYMENT.md`](./DEPLOYMENT.md) | Guía detallada paso a paso para el despliegue a producción. | Markdown |
...
El directorio [`/cli`](./cli) contiene una útil terminal interactiva en Python que se conecta a la API.
...
Para instrucciones detalladas sobre el despliegue, variables de producción y sincronización de datos vectoriales, consulta la **[Guía de Despliegue de Producción (DEPLOYMENT.md)](./DEPLOYMENT.md)**.
