const mysql = require('C:/PitayaCode/acuacore/api/node_modules/mysql2/promise');
...
async function main() {
  ...
  try {
    ...
    if (tenants.length > 0) {
      ...
      await connection.execute("UPDATE User SET tenantId = ? WHERE email = 'admin@pitayacode.io'", [tid]);
